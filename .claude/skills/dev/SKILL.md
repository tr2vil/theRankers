...
### Phase 6: 테스트 레지스트리 갱신
## !! 경고: 이 Phase는 절대 생략/누락 불가 !!
## !! 구현(Phase 2) 직후, 테스트 실행(Phase 3) 이전에 반드시 수행 !!
## !! 시험 항목 추가 없이 완료 보고를 하면 안 됨 !!

개발에서 코드를 변경했다면, 판단하에 해당 변경에 대한 Playwright 테스트를 추가한다.
만약 판단하에 테스트를 추가할것이 없다면 반드시 이를 알리도록 한다.
— API가 변경되면 API 테스트, 페이지가 변경되면 페이지 테스트, 로직이 변경되면 로직 검증 테스트를 반드시 추가한다.

**절차:**

1. **시험 항목 작성**: 이번 개발에서 추가/변경된 기능에 대한 Playwright 테스트 코드를 작성
   - **Backend API 추가/변경 시**: `e2e/tests/` 해당 파일에 엔드포인트 응답 테스트 추가
...
2. **레지스트리 등록**: `.claude/test-registry.md`에 새 시험 항목 ID, 설명을 추가
3. **ID 규칙**: `T{카테고리번호}-{항목번호}` 형식 유지 (예: T01-8, T03-14, T05-11)
4. **전체 테스트 실행**: 새 테스트 포함 전체 Playwright 테스트 실행 → HTML 리포트 생성 → 브라우저 표시
5. **레지스트리 상태 갱신**: 실행 결과에 따라 PASS/FAIL 상태 업데이트
...
2. **Phase 6 (시험 항목 추가)는 절대 생략 불가** — 개발 후 Playwright 테스트 미추가 시 완료로 간주하지 않음
3. 프론트엔드 변경 포함 시 `/ux-reviewer`도 **필수**
4. Docker가 내려가 있으면 사용자에게 시작 요청
5. 최대 3회 반복 순환 후 사용자에게 보고
6. "완료"는 PASS + APPROVE 이후에만 사용
7. 근본 원인 없이 코드 변경 금지 (추측 금지)
8. 데이터 수집 로직 변경 시 반드시 중복 방지 검증
9. 풀스택 작업 시 **백엔드 API 먼저, 프론트엔드 나중에**
